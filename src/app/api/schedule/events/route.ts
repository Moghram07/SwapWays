...
export async function GET(request: Request) {
  ...
  try {
    const [schedule, trades, matches] = await withTimeout(
      Promise.all([
    ...
    console.error("[schedule/events] degraded response due to transient DB failure", error);
    const days = buildCalendarData([], month, year).map((day) => ({
      ...day,
      date: day.date.toISOString(),
    }));
    return NextResponse.json(
      {
        data: { events: [], month, year, days },
        error: "ServiceUnavailable",
        message: "Calendar data is temporarily unavailable. Please refresh in a moment.",
      },
      { status: 200 }
    );
  }
