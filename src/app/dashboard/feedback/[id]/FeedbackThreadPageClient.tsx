    ...
    <section className="mx-auto max-w-3xl space-y-4">
      ...
      <div className="rounded-xl border border-line bg-surface p-4">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-surface-2 px-2 py-0.5 text-xs text-content-soft">{ticket.type}</span>
          <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs text-blue-700">{ticket.status}</span>
        </div>
        <h1 className="mt-2 text-lg font-semibold text-content">{ticket.subject || "(no subject)"}</h1>
        <p className="text-xs text-muted">Submitted {formatDateShort(ticket.createdAt)}</p>
      </div>
