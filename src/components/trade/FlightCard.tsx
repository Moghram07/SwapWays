...
export function FlightCard({ trade, showSwapButton = true }: FlightCardProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 rounded-xl border border-line bg-surface p-4 shadow-sm">
      <div
      ...
      <div className="min-w-0 flex-1">
        <p className="font-semibold text-content">{route}</p>
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-6">
        <div>
          ...
          <p className="font-medium text-content">{formatDate(new Date(trade.departureDate))}</p>
        </div>
        <div>
          <p className="text-xs text-muted">Departure</p>
          <p className="font-medium text-content">{departureTime}</p>
        </div>
        <div>
          <p className="text-xs text-muted">Duration</p>
          <p className="font-medium text-content">{duration}</p>
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-2">
        <span
          className={`rounded-full px-3 py-1 text-xs font-medium ${
            hasPendingMatch ? "text-white" : trade.status === "OPEN" ? "bg-emerald-50 text-emerald-700" : "bg-surface-2 text-muted"
          }`}
