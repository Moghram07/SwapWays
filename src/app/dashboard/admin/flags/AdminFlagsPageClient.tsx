...
export function AdminFlagsPageClient() {
  ...
  return (
    <section className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-content">Flagged Accounts</h1>
        <p className="text-sm text-muted">Review suspicious activity flags and resolve them.</p>
      </div>

      {error && <p className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">{error}</p>}
      {loading ? (
        <p className="text-sm text-muted">Loading flags...</p>
      ) : items.length === 0 ? (
        <p className="rounded-xl border border-line bg-surface p-6 text-sm text-muted">No unresolved flags.</p>
      ) : (
            ...
            <article key={flag.id} className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-semibold text-content">
                  {flag.user.firstName} {flag.user.lastName} <span className="font-normal text-muted">{flag.user.email}</span>
                </p>
                <span className="rounded-full bg-amber-200 px-2 py-0.5 text-xs font-medium text-amber-800">{flag.type}</span>
              </div>
              <p className="mt-2 text-sm text-content-soft">{flag.details}</p>
